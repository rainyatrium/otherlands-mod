    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        if (!world.isClient) {
            ...
            if (rand.nextInt(101) == 0) {
                world.playSound(null, userpos, Otherlands.bonersecret, SoundCategory.MASTER, volume, pitch);
                user.sendMessage(new LiteralText("PENIS!").formatted(Formatting.GOLD), false);
            } else {
                world.playSound(null, userpos, Otherlands.boner, SoundCategory.MASTER, volume, pitch);
